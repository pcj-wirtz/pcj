...
--SQLServerManager14.msc para SQL Server 2019
...
INSERT INTO images(id, imageFile)
		...
		FROM OPENROWSET(BULK 'C:\BLOB\brad.jfif', SINGLE_BLOB) as f;
GO
INSERT INTO images(id, imageFile)
	SELECT NEWID(), BulkColumn
	FROM OPENROWSET(BULK 'C:\BLOB\tom.jfif', SINGLE_BLOB) as f;
GO
INSERT INTO images(id, imageFile)
	SELECT NEWID(), BulkColumn
	FROM OPENROWSET(BULK 'C:\BLOB\will.jfif', SINGLE_BLOB) as f;
GO
...
-- https://docs.microsoft.com/es-es/sql/relational-databases/blob/create-client-applications-for-filestream-data?view=sql-server-2019


-- https://docs.microsoft.com/es-es/sql/relational-databases/blob/access-filestream-data-with-opensqlfilestream?view=sql-server-2019
...
-- CONTAINMENT = NONE
-- ON  PRIMARY 
--( NAME = N'AdventureWorks2019', FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL14.MSSQLSERVER\MSSQL\DATA\AdventureWorks2019.mdf' , SIZE = 270336KB , MAXSIZE = UNLIMITED, FILEGROWTH = 65536KB )
-- LOG ON 
--( NAME = N'AdventureWorks2019_log', FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL14.MSSQLSERVER\MSSQL\DATA\AdventureWorks2019_log.ldf' , SIZE = 73728KB , MAXSIZE = 2048GB , FILEGROWTH = 65536KB )
--GO
-------------------
...
ALTER DATABASE [AdventureWorks2019]
       ADD FILE (
             NAME = '[AdventureWorks2019]_filestream',
             FILENAME = 'C:\Program Files\Microsoft SQL Server\MSSQL15.MSSQLSERVER\MSSQL\DATA\AdventureWorks2019_FS'
       )
       TO FILEGROUP [PRIMARY_FILESTREAM]
GO
...
--Default FILESTREAM filegroup is not available in database 'AdventureWorks2019'.
